...
def noti_add(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, (title + ' Ha sido subido.'))
            return redirect('home')
        else:
            messages.error(request, 'Por favor corrija los siguientes errores.')
    else:
        form = NewsAndEventsForm()
    return render(request, 'app/noti_add.html', {
        'title': 'Agregar Publicacion | Django',
        'form': form,
    })
...
def edit_post(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, (title + ' Ha sido actualizado.'))
            return redirect('home')
        else:
            messages.error(request, 'Por favor corrija los siguientes errores.')
    else:
    ...
    return render(request, 'app/noti_add.html', {
        'title': 'Editar Publicacion | Django',
        'form': form,
    })
...
def delete_post(request, pk):
    ...
    title = post.title
    post.delete()
    messages.success(request, (title + ' ha sido eliminado.'))
    return redirect('home')
...
def session_add_view(request):
    """ método de solicitud de verificación, si POST agregamos sesión; de lo contrario, mostraremos el formulario vacío """
    if request.method == 'POST':
...
@login_required
@lecturer_required
def gen_pdf_view(request):
    semesters = Semester.objects.all().order_by('-is_current_semester', '-semester')
    return render(request, 'app/gen_pdf.html', {"semesters": semesters, })
...
def semester_add_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            data = form.data.get('is_current_semester')  # devuelve una cadena de 'Verdadero' si el usuario seleccionó Sí
            if data == 'True':
...
def semester_update_view(request, pk):
    ...
    if request.method == 'POST':
        if request.POST.get('is_current_semester') == 'True': #devuelve una cadena de 'Verdadero' si el usuario seleccionó sí para 'es el semestre actual'
            unset_semester = Semester.objects.get(is_current_semester=True)
            unset_semester.is_current_semester = False
            unset_semester.save()
            unset_session = Session.objects.get(is_current_session=True)
